...
void smtcheck_opensmt2t_cuf::initializeSolver()
{
  ...
  literals[0] = cuflogic->getTerm_true(); // Which is .x =0
  // KE: End of fix
}

// Free all resources related to OpenSMT2
void smtcheck_opensmt2t_cuf::freeSolver()
{
...
Function: smtcheck_opensmt2t_cuf::new_partition
...
\*******************************************************************/
fle_part_idt smtcheck_opensmt2t_cuf::new_partition()
{
...
literalt smtcheck_opensmt2t_cuf::new_variable()
{
  literalt l;
...
Function: smtcheck_opensmt2t_cuf::push_variable
...
\*******************************************************************/
literalt smtcheck_opensmt2t_cuf::push_variable(PTRef ptl) {
...
exprt smtcheck_opensmt2t_cuf::get_value(const exprt &expr)
{
	...
	if (converted_exprs.find(expr.hash()) != converted_exprs.end()) {
		...
		if (cuflogic->isIteVar(ptrf)) // true/false - evaluation of a branching
		{
		...
		else if (cuflogic->isTrue(ptrf)) //true only
		{
			return true_exprt();
		}
		else if (cuflogic->isFalse(ptrf)) //false only
		{
			return false_exprt();
		}
		else if (cuflogic->isVar(ptrf)) // Constant value
		{
			// Create the value
			ValPair v1 = mainSolver->getValue(ptrf);
		...
		else if (cuflogic->isConstant(ptrf))
		{
			// Constant?
			ValPair v1 = mainSolver->getValue(ptrf);
			irep_idt value = v1.val;
...
bool smtcheck_opensmt2t_cuf::is_assignemt_true(literalt a) const
{
    if (a.is_true())
        return true;
    else if (a.is_false())
...
PTRef smtcheck_opensmt2t_cuf::convert_symbol(const exprt &expr)
{
...
literalt smtcheck_opensmt2t_cuf::const_var(bool val)
{
    literalt l;

    PTRef c = val ? cuflogic->getTerm_true() : cuflogic->getTerm_false();
...
literalt smtcheck_opensmt2t_cuf::type_cast(const exprt &expr) {
    ...
    if (expr.is_boolean() && (expr.operands())[0].is_constant()) {
        ...
        PTRef ptl = cuflogic->mkIte(literals[lt.var_no()], cuflogic->mkConst(1), cuflogic->mkConst(0));
        ...
        literalt lt = convert((expr.operands())[0]); // Creating the Bool expression
        PTRef ptl = cuflogic->mkNot(cuflogic->mkEq(literals[lt.var_no()], cuflogic->mkConst(0)));
        l = push_variable(ptl); // Keeps the new literal + index it
    } else {
...
//        return converted_exprs[expr.hash()];
    ...
    if (expr.id()==ID_symbol || expr.id()==ID_nondet_symbol) {
        ...
        if (expr.id()==ID_notequal) {
            ptl = cuflogic->mkNot(cuflogic->mkEq(args));
        } else if (expr.id() == ID_equal) {
            ptl = cuflogic->mkEq(args);
        } else if (expr.id()==ID_if) {
            ptl = cuflogic->mkIte(args);
#ifdef DEBUG_SMT_LRA
            ite_map_str.insert(make_pair(string(getPTermString(ptl)), cuflogic->printTerm(cuflogic->getTopLevelIte(ptl))));
#endif
            ...
            ptl = cuflogic->mkAnd(args);
        } else if (expr.id() == ID_or) {
            ptl = cuflogic->mkOr(args);
        } else if (expr.id() == ID_not) {
            ptl = cuflogic->mkNot(args);
        } else if (expr.id() == ID_implies) {
            ptl = cuflogic->mkImpl(args);
...
literalt smtcheck_opensmt2t_cuf::lunsupported2var(exprt expr)
{
    ...
    if (expr.is_boolean())
        var = cuflogic->mkBoolVar(str.c_str());
    else
...
void smtcheck_opensmt2t_cuf::set_to_true(PTRef ptr)
{
...
void smtcheck_opensmt2t_cuf::set_to_true(const exprt &expr)
{
    literalt l = convert(expr);
    PTRef lp = literals[l.var_no()];
    PTRef truep = cuflogic->getTerm_true();
    vec<PTRef> args;
    args.push(lp);
    args.push(truep);
    PTRef tlp = cuflogic->mkEq(args);

    assert(tlp != PTRef_Undef);
    current_partition->push(tlp);
}

void smtcheck_opensmt2t_cuf::set_to_false(const exprt &expr)
{
    literalt l = convert(expr);
    PTRef lp = literals[l.var_no()];
    PTRef falsep = cuflogic->getTerm_false();
    vec<PTRef> args;
    args.push(lp);
    args.push(falsep);
    PTRef tlp = cuflogic->mkEq(args);
...
void smtcheck_opensmt2t_cuf::set_equal(literalt l1, literalt l2){
    vec<PTRef> args;
    literalt l;
...
literalt smtcheck_opensmt2t_cuf::limplies(literalt l1, literalt l2){
    literalt l;
    vec<PTRef> args;
    PTRef pl1 = literals[l1.var_no()];
    PTRef pl2 = literals[l2.var_no()];
    args.push(pl1);
    args.push(pl2);
    PTRef ans = cuflogic->mkImpl(args);
    l = push_variable(ans); // Keeps the new PTRef + create for it a new index/literal
...
literalt smtcheck_opensmt2t_cuf::land(literalt l1, literalt l2) {
    literalt l;
    vec<PTRef> args;
    PTRef pl1 = literals[l1.var_no()];
    PTRef pl2 = literals[l2.var_no()];
    args.push(pl1);
    args.push(pl2);
    PTRef ans = cuflogic->mkAnd(args);
    l = push_variable(ans); // Keeps the new PTRef + create for it a new index/literal

    return l;
}

literalt smtcheck_opensmt2t_cuf::land(bvt b){
    literalt l;
    vec<PTRef> args;
...
literalt smtcheck_opensmt2t_cuf::lor(literalt l1, literalt l2){
    literalt l;
    vec<PTRef> args;
    PTRef pl1 = literals[l1.var_no()];
    PTRef pl2 = literals[l2.var_no()];
    args.push(pl1);
    args.push(pl2);
    PTRef ans = cuflogic->mkOr(args);
    l = push_variable(ans); // Keeps the new PTRef + create for it a new index/literal

    return l;
}

literalt smtcheck_opensmt2t_cuf::lor(bvt b){
    literalt l;
    vec<PTRef> args;
    for (bvt::iterator it = b.begin(); it != b.end(); ++it)
    {
...
literalt smtcheck_opensmt2t_cuf::lnot(literalt l){
    literalt ln;
    vec<PTRef> args;
    PTRef pl1 = literals[l.var_no()];
    args.push(pl1);
    PTRef ans = cuflogic->mkNot(args);
...
literalt smtcheck_opensmt2t_cuf::lconst(const exprt &expr)
{
    literalt l;
    if (expr.is_boolean()) {
...
void smtcheck_opensmt2t_cuf::extract_itp(PTRef ptref, smt_itpt& itp) const
{
    //ptref_cachet cache;
...
literalt smtcheck_opensmt2t_cuf::extract_itp_rec(PTRef ptref,
  prop_itpt& target_itp, ptref_cachet& ptref_cache) const
{
    ...
    if (cuflogic->getTerm_true() == ptref) {
        result = const_literal(true);
    }
    else if (cuflogic->getTerm_false() == ptref) {
        result = const_literal(false);
    }
    else if (cuflogic->isAnd(ptref))
    {
        Pterm& ptm = cuflogic->getPterm(ptref);
        assert(ptm.size() >= 2);
    ...
    {
        Pterm& ptm = cuflogic->getPterm(ptref);
        cout << "; error " << cuflogic->getSymName(ptref) << endl;
        assert(ptm.size() == 0);
    }
...
void smtcheck_opensmt2t_cuf::produceConfigMatrixInterpolants (const vector< vector<int> > &configs, vector<PTRef> &interpolants)
{
...
string
smtcheck_opensmt2t_cuf::unquote_varname(const string& varname)
{
...
string
smtcheck_opensmt2t_cuf::insert_index(const string& _varname, int _idx)
{
...
string
smtcheck_opensmt2t_cuf::insert_index(const string& _varname, const string& _idx)
{
    string unidx = remove_index(_varname);
    string varname = unquote_varname(unidx);
...
int
smtcheck_opensmt2t_cuf::get_index(const string& _varname)
{
...
string
smtcheck_opensmt2t_cuf::quote_varname(const string& varname)
{
...
void
smtcheck_opensmt2t_cuf::adjust_function(smt_itpt& itp, std::vector<symbol_exprt>& common_symbs, string _fun_name, bool substitute)
{
    ...
    for (vector<string>::iterator it = quoted_varnames.begin(); it != quoted_varnames.end(); ++it)
    {
        ...
        PTRef new_var = cuflogic->mkVar(cuflogic->getSortRef(var), new_var_name.c_str());
    ...
    if (substitute)
    {
        cuflogic->varsubstitute(itp_pt, subst, new_root);
        //cout << "; Formula " << cuflogic->printTerm(itp.getInterpolant()) << " is now " << cuflogic->printTerm(new_root) << endl;
    }
    else
    {
        new_root = cuflogic->getTerm_true();
    }
    tterm->setBody(new_root);

    tterm->setName(fun_name);
    //cuflogic->addFunction(tterm);
    //cuflogic->dumpFunction(cout, tterm);
    itp.setTterm(*tterm);
    itp.setLogic(cuflogic);
}

void
smtcheck_opensmt2t_cuf::fill_vars(PTRef itp, map<string, PTRef>& subst)
{
    set<PTRef> visited;
    queue<PTRef> q;
    q.push(itp);
    while (!q.empty())
    {
        ...
        if (visited.find(p) != visited.end())
            continue;
        if (cuflogic->isVar(p))
            subst[cuflogic->getSymName(p)] = p;
        else
        {
            Pterm& pt = cuflogic->getPterm(p);
...
bool
smtcheck_opensmt2t_cuf::is_quoted_var(const string& varname)
{
    assert(varname.length() > 0);
...
string
smtcheck_opensmt2t_cuf::remove_invalid(const string& varname)
{
    string ans("");
...
string
smtcheck_opensmt2t_cuf::remove_index(string var)
{
    int i = var.length() - 1;
    while (i >= 0 && var[i] != '#') --i;
    string no_index;
...
Function: smtcheck_opensmt2t_cuf::get_interpolant
...
void smtcheck_opensmt2t_cuf::get_interpolant(const interpolation_taskt& partition_ids, interpolantst& interpolants)
{
    ...
    for (unsigned i = 0; i < itp_ptrefs.size(); ++i)
    {
        ...
        char *s = cuflogic->printTerm(interpolants.back()->getInterpolant());
#ifdef DEBUG_SMT_ITP
...
Function: smtcheck_opensmt2t_cuf::can_interpolate
...
\*******************************************************************/
bool smtcheck_opensmt2t_cuf::can_interpolate() const
{
...
Function: smtcheck_opensmt2t_cuf::prop_solve
...
bool smtcheck_opensmt2t_cuf::solve() {
...
#ifdef DEBUG_SMT_EUF
    cuflogic->dumpHeaderToFile(cout);
#endif
    //  add_variables();
    char *msg;
    for (int i = pushed_formulas; i < top_level_formulas.size(); ++i) {
#ifdef DEBUG_SMT_EUF
        char* s = cuflogic->printTerm(top_level_formulas[i]);
        cout << "\n(assert\n" << s << "\n)" << endl;
        free(s);
#endif
...
#ifdef DEBUG_SMT2SOLVER
    dump_on_error("smtcheck_opensmt2t_cuf::solve::1082"); // To print current code in the solver
#endif
...
Function: smtcheck_opensmt2t_cuf::close_partition
...
void smtcheck_opensmt2t_cuf::close_partition()
{
    assert(current_partition != NULL);
    if (partition_count > 0) {
        if (current_partition->size() >= 1) {
            PTRef pand = cuflogic->mkAnd(*current_partition);
#ifdef DEBUG_SMT2SOLVER
            char* s= cuflogic->printTerm(pand);
            ...
            top_level_formulas.push(cuflogic->getTerm_true());
        } */
    }
...
Function: smtcheck_opensmt2t_cuf::getVars
...
Function: smtcheck_opensmt2t_cuf::extract_expr_str_number
...
\*******************************************************************/
std::string smtcheck_opensmt2t_cuf::extract_expr_str_number(const exprt &expr)
{
...
Function: smtcheck_opensmt2t_cuf::extract_expr_str_name
...
\*******************************************************************/
std::string smtcheck_opensmt2t_cuf::extract_expr_str_name(const exprt &expr)
{
...
Function: smtcheck_opensmt2t_cuf::dump_on_error
...
\*******************************************************************/
void smtcheck_opensmt2t_cuf::dump_on_error(std::string location) {
    ...
    for (int i = 0; i < top_level_formulas.size(); ++i) {
        PTRef tmp;
        cuflogic->conjoinItes(top_level_formulas[i], tmp);
        char *s = cuflogic->printTerm(tmp);
