...
#ifndef CPROVER_SMTCHECK_Z3_LRA_H
#define CPROVER_SMTCHECK_Z3_LRA_H

#include "smtcheck_z3.h"

class smtcheck_z3_lrat : public smtcheck_z3t
{
public:
    // Defualt C'tor, data to init are from smt_params class in Z3, token are in smt_params_helper
    smtcheck_z3_lrat(const solver_optionst solver_options) :
            smtcheck_z3t(solver_options, "QF_LRA"),
            m_ptr_assert_var_constraints(nullptr),
            m_type_constraints(solver_options.m_type_constraints)
    { initialize_solver(); }

    virtual ~smtcheck_z3_lrat() {} // d'tor
    
    virtual FlaRef get_and_clear_var_constraints() override;
    
    virtual z3::sort get_numeric_sort() override { return m_query_context.real_sort(); }
...
#endif // CPROVER_SMTCHECK_Z3_LRA_H
