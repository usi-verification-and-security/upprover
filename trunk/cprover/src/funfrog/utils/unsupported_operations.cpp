...
\*******************************************************************/
std::string unsupported_operationst::create_new_unsupported_var(std::string type_name, bool no_rename, bool no_quote)
{
    // Create a new unsupported va
    std::string str = unsupported_symbol(type_name);
    assert(str.size() > 0); 
    
    str = no_rename ? str : (str+"!0#"+ std::to_string(unsupported2var++));
    return (no_quote ? str : quote_if_necessary(str));
}
